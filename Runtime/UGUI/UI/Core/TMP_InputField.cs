...
{
	[AddComponentMenu("MultiWindowUI/TextMeshPro - Input Field", 11)]
		...
		IScrollHandler
	{
		...
		private BaseInput inputSystem
		{
			get
			{
				if (eventSystem && eventSystem.currentInputModule)
					return eventSystem.currentInputModule.input;
				return null;
			}
		}
		...
		public UnityEngine.UI.Graphic placeholder { get { return m_Placeholder; } set { SetPropertyUtility.SetClass(ref m_Placeholder, value); } }
		...
		/// </summary>
		public TMPro.TMP_InputValidator inputValidator
		{
		...
		[SerializeField]
		protected TMPro.TMP_InputValidator m_InputValidator = null;
		...
		protected override void OnEnable()
		{
			...
			UnityEngine.UI.ILayoutController layoutController = GetComponent<UnityEngine.UI.ILayoutController>();

			if (layoutController != null)
			{
				m_IsDrivenByLayoutComponents = true;
				m_LayoutGroup = GetComponent<UnityEngine.UI.LayoutGroup>();
			}
			else
			...
			if (Application.isPlaying)
			{
				...
				{
					GameObject go = new GameObject("Caret", typeof(TMPro.TMP_SelectionCaret));
			...
			TMPro.TMPro_EventManager.TEXT_CHANGED_EVENT.Add(ON_TEXT_CHANGED);
		}
		...
		protected override void OnDisable()
		{
			...
			TMPro.TMPro_EventManager.TEXT_CHANGED_EVENT.Remove(ON_TEXT_CHANGED);
		...
		public virtual void OnDrag(PointerEventData eventData)
		{
			...
			TMPro.CaretPosition insertionSide;

			int insertionIndex = TMPro.TMP_TextUtilities.GetCursorIndexFromPosition(m_TextComponent, eventData.position, eventData.pressEventCamera, out insertionSide);
		...
		public override void OnPointerDown(PointerEventData eventData)
		{
			...
			if (hadFocusBefore || !m_OnFocusSelectAll)
			{
				...
				if (isDoubleClick)
				{
					int wordIndex = TMPro.TMP_TextUtilities.FindIntersectingWord(m_TextComponent, eventData.position, eventData.pressEventCamera);
		...
		private int LineUpCharacterPosition(int originalPos, bool goToFirstChar)
		{
			...
			TMPro.TMP_CharacterInfo originChar = m_TextComponent.textInfo.characterInfo[originalPos];
			...
			int closest = -1;
			float distance = TMPro.TMP_Math.FLOAT_MAX;
			float range = 0;

			for (int i = m_TextComponent.textInfo.lineInfo[originLine - 1].firstCharacterIndex; i < endCharIdx; ++i)
			{
				TMPro.TMP_CharacterInfo currentChar = m_TextComponent.textInfo.characterInfo[i];
		...
		protected void UpdateLabel()
		{
			if (m_TextComponent != null && m_TextComponent.font != null && m_PreventCallback == false)
			{
				...
				if (m_IsDrivenByLayoutComponents)
					UnityEngine.UI.LayoutRebuilder.MarkLayoutForRebuild(m_RectTransform);
		...
		private void GenerateCaret(UnityEngine.UI.VertexHelper vbo, Vector2 roundingOffset)
		{
			...
			float height = 0;
			TMPro.TMP_CharacterInfo currentCharacter;
			...
			if (caretPositionInternal == m_TextComponent.textInfo.lineInfo[currentLine].firstCharacterIndex)
			{
				...
				if (m_TextComponent.verticalAlignment == TMPro.VerticalAlignmentOptions.Geometry)
			...
			TMPro.TMP_FontAsset fontAsset = m_TextComponent.font;
		...
		private void GenerateHighlight(UnityEngine.UI.VertexHelper vbo, Vector2 roundingOffset)
		{
			// Update Masking Region
			UpdateMaskRegions();
			...
			while (currentChar <= endChar && currentChar < textInfo.characterCount)
			{
				...
				{
					TMPro.TMP_CharacterInfo startCharInfo = textInfo.characterInfo[startChar];
					TMPro.TMP_CharacterInfo endCharInfo = textInfo.characterInfo[currentChar];
		...
		void SetTextComponentWrapMode()
		{
			...
			if (multiLine)
				m_TextComponent.textWrappingMode = TMPro.TextWrappingModes.Normal;
			else
				m_TextComponent.textWrappingMode = TMPro.TextWrappingModes.PreserveWhitespaceNoWrap;
		}
		...
		{
			TMPro.TMP_Text placeholderTextComponent = m_Placeholder as TMPro.TMP_Text;
		...
		public void SetGlobalFontAsset(TMPro.TMP_FontAsset fontAsset)
		{
			TMPro.TMP_Text placeholderTextComponent = m_Placeholder as TMPro.TMP_Text;
